...
██║     ███████║██║  ██║ ╚████╔╝ ███████╗██║  ██║
╚═╝     ╚══════╝╚═╝  ╚═╝  ╚═══╝  ╚══════╝╚═╝  ╚═╝
مراقب Webserver — by PID | Ctrl+C للخروج
"""
...
try:
    ...
    sys.exit("❌  psutil غير مثبت — شغّل:  pip install psutil rich")
...
try:
    ...
    sys.exit("❌  rich غير مثبت — شغّل:  pip install rich")


# ─── إعدادات ──────────────────────────────────────────────────────────────────
HISTORY_LEN   = 60          # عدد نقاط الـ sparkline
REFRESH_HZ    = 1.0         # مرات التحديث في الثانية
ALERT_CPU     = 85.0        # % CPU — حد التحذير
ALERT_MEM_MB  = 500.0       # MB  RAM — حد التحذير
ALERT_FDS     = 1000        # عدد file descriptors — حد التحذير
SPARKLINE_CHARS = "▁▂▃▄▅▆▇█"
...
class ServerStats:
    ...
    def collect(self):
        ...
        try:
            ...
            # شبكة — نسبة للنظام (عملية واحدة لا تملك io_counters network)
...
def main():
    ...
    # تحديث أولي بدون interval (لتفادي 0.0 في القراءة الأولى)
    stats.collect()
    time.sleep(0.5)
    stats.collect()
