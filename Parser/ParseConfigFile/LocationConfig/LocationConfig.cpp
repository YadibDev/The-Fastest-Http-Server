...
bool    clsLocation::ParseRoot()
{
    ...
    _root.initView(); // Mandatory for s_view safety
...
bool    clsLocation::ParseAlias()
{
    ...
    _alias.raw_path = ConfigDirectiveParser::parseAlias(ctx);
    _alias.initView();
    return !ctx.error.isError();
}
...
bool    clsLocation::ParseCgiPass()
{
    ...
    for (it = tmpCgi.begin(); it != tmpCgi.end(); ++it) {
        s_uri_entry entry;
        entry.raw_path = it->second;
        entry.initView();
        _cgi_pass[it->first] = entry;
    }
    return !ctx.error.isError();
}
...
clsLocation::clsLocation(s_parse_context& ctxs, const std::string &sRoot
    ...
    : ctx(ctxs)
{
    _root.raw_path = sRoot;
    _root.initView();
    
    for (size_t i = 0; i < sIndex.size(); ++i) {
        s_uri_entry entry;
        entry.raw_path = sIndex[i];
        entry.initView();
        _index.push_back(entry);
    }
...
const s_uri_entry &clsLocation::getRoot() const { return _root; }
const s_uri_entry &clsLocation::getAlias() const { return _alias; }
...
const std::map<std::string, s_uri_entry> &clsLocation::getCgiPass() const { return _cgi_pass; }
