...
private:
    ...
    std::string _known_headers[header_id::H_KNOWN_COUNT];

    uint32_t    _header_mask;
    
    std::map<std::string, std::string> _unknownHeaders;

    header_id::e_header_id _getHeaderID(const std::string& key) const;
    bool        _is_singleton(header_id::e_header_id id) const;
...
public:
    ...
    const std::string& getHeader(header_id::e_header_id id) const;
    const std::map<std::string, std::string>& getUnknownHeaders() const;
    uint32_t getHeaderMask() const;
    
    bool hasHeader(header_id::e_header_id id) const { return (_header_mask & (1 << id)); }
};
