...
const char* CreateENVCGI::_knownHeaderNames[H_KNOWN_COUNT] = {
    "host", "content-length", "content-type", "transfer-encoding", 
    "connection", "expect", "user-agent", "cookie"
};
...
size_t CreateENVCGI::_calculateEnvSize(const clsHeader& header) {
    ...
    for (int i = 0; i < H_KNOWN_COUNT; ++i) {
        if ((mask & (1 << i)) && i != H_CONTENT_TYPE && i != H_CONTENT_LENGTH)
            knownCount++;
    }
...
void CreateENVCGI::_addBaseEnvVars(char** envp, size_t& idx, const std::string& path, 
    ...
    envp[idx++] = strdup(("CONTENT_TYPE=" + header.getHeader(H_CONTENT_TYPE)).c_str());
    envp[idx++] = strdup(("CONTENT_LENGTH=" + header.getHeader(H_CONTENT_LENGTH)).c_str());
}
...
void CreateENVCGI::_addHeaderEnvVars(char** envp, size_t& idx, const clsHeader& header) {
    ...
    for (int i = 0; i < H_KNOWN_COUNT; ++i) {
        if ((mask & (1 << i)) && i != H_CONTENT_TYPE && i != H_CONTENT_LENGTH) {
            ...
            envp[idx++] = strdup((key + "=" + header.getHeader(static_cast<e_header_id>(i))).c_str());
        }
    }
