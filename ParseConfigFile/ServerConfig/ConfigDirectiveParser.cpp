...
short ConfigDirectiveParser::parseMethods(s_parse_context& ctx) {
    ...
    if (ctx.parser.peek().type != TOKEN_SEMICOLON)
        ctx.error.setStatus(400, "Expected ';' after methods");
    else
        ctx.parser.advance();
...
sockaddr_in ConfigDirectiveParser::setSockaddr_in(const std::string& input, s_parse_context& ctx) {
...
uint32_t ConfigDirectiveParser::validateIPWithSystem(const std::string& ip, int family, s_parse_context& ctx) {
    uint32_t ipv4_raw;
